  ...
  catchAsyncErrors(async (req, res, next) => {
    ...
    try {
      ...
      for (const rm of order.rawMaterialRequired) {
        ...
        // Create a proper MaterialOutward record with full reference
        await MaterialOutward.create([{
          ...
          remarks:     `Order approval — Order #${order.orderNo ?? ""}`,
        }], { session });
      }
